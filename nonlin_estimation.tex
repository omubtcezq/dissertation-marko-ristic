...
\begin{algorithmic}[1]
    ...
    \For{$\epsilon \gets 1$ to $6$}
        ...
        \State Compute $\mathsf{E}^{-1}_{1}(\mathcal{D}_{\mathsf{sk}_{\mathsf{a},0}}(\bm{\alpha}^{(k,\epsilon)}))$
    \EndFor
...
\begin{equation}\label{eq:nonlin_fusion:localisation_leakage}
    \sum_{i=1}^nz'_{k,i}\,,\ \sum_{i=1}^nr'_{k,i}\,,\ \sum_{i=1}^ns_{x,i}\ \textrm{and}\ \sum_{i=1}^ns_{y,i}\,,
\end{equation}
