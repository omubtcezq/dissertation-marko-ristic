...
\begin{description}
    ...
    \end{equation*}
    for weights $\theta^{(t^*)}_j$, $1\leq j\leq l$, returned by a \textit{Weight Query} with chosen instance $t^*$. The challenger then chooses a random bit $b \in \{1,0\}$ and returns encryptions 
    \begin{equation*}
        \left\langle\mathcal{E}_{\mathsf{pk}_{\mathsf{a}},\mathsf{sk}_{\mathsf{a}, i}}\left(\sum^l_{j=1}a^{(t^*)(b)}_{i,j}\theta^{(t^*)}_j\right)\ \middle|\ i\in S\right\rangle\,.
    \end{equation*}
    ...
    \item[Guess] At the end, the attacker outputs a bit $b^\prime$ and wins the game if and only if $b^\prime = b$. The advantage of an attacker $\mathcal{A}$ is defined as
    \begin{equation*}
        \mathsf{Adv}^{LCAO}(\mathcal{A}) \coloneqq \left\lvert \Pr [b^\prime=b] - \frac{1}{2}\right\rvert\,.
    \end{equation*} 
\end{description}
...
\begin{proof}
    ...
    \begin{description}
        ...
        \begin{equation*}
            \left\langle(N+1)^{\sum^l_{j=1}a^{(t^*)(b)}_{i,j}\theta^{(t^*)}_j}H(t^*)^{\mathsf{sk}_{\mathsf{a},i}}\ \middle|\ i\in U\right\rangle\,,
        \end{equation*}
        for an unknown $b \in \{0,1\}$. Multiply series elements by $\tilde{\rho}_i^N$, $1\leq i\leq n$, for randomly chosen $\tilde{\rho}_i \in \mathbb{Z}_N$ and return
        \begin{equation*}
            \left\langle(N+1)^{\sum^l_{j=1}a^{(t^*)(b)}_{i,j}\theta^{(t^*)}_j}\tilde{\rho}_i^N H(t^*)^{\mathsf{sk}_{\mathsf{a},i}}\ \middle|\ i\in U\right\rangle
        \end{equation*}
        to $\mathcal{A}$.
