...
% Additionally, we note that the current recommended key length for the Paillier encryption scheme is $2048$ bits [barkerRecommendationPairWiseKey2019], easily supporting a modulus $N$ and fractional precision $\phi$ that guarantee similar performance.
...
Key generation is computed by choosing two equal-length and sufficiently large primes $p$ and $q$, and computing $N=pq$. A hash function $H:\mathbb{Z} \rightarrow \mathbb{Z}_{N^2}^*$ is defined and the public key is set to $(N, H)$. $n$ private keys are generated by choosing $\mathsf{sk}_i,\,i\in\{1,\dots,n\}$, uniformly from $\mathbb{Z}_{N^2}$ and distributing them to $n$ participants (whose values are to be aggregated), while the last key is set as
\begin{equation}
...
\begin{equation}
    c^{(k)}_{i} = (N+1)^{a^{(k)}_{i}} H(k)^{\mathsf{sk}_i} \pmod{N^2}\,.
\end{equation}
...
\begin{equation}
    c^{(k)} = H(k)^{\mathsf{sk}_0}\prod^{n}_{i=1}c^{(k)}_{i} \pmod{N^2} \label{eq:prelims:joye_libert_agg_sum}
\end{equation}
and
\begin{equation}
