...
Sensor and estimate data that we consider is primarily Bayesian in nature and typically consists of estimates and associated estimate uncertainties. The linear Kalman filter (KF) and the linearising extended Kalman filter (EKF), along with their information filter equivalents, are particularly useful in the estimation and fusion of such data. A general fusion algorithm, the covariance intersection (CI), used when data cross-correlations are unknown, is also introduced.
...
Unlike the linear KF, optimality is not guaranteed with the EKF due to the approximation of 
A highly non-linear process or measurement function would naturally lead to greater errors
