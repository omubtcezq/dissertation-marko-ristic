...
The details of the confidential estimate fusion problem that we consider are
...
% TODO method that allows full computation of the fusion results on the cloud at the cost of relative sensor performance leakage. This leakage, however, is entirely a downside. In bandwidth-limited environments, a preference for communication with better sensors is beneficial for estimation and therefore the accuracy of the final fusion results.
% The method, secfci, makes use of paillier and ore, with the only leakage being enherntly captures by the leakage expected from the using ore. Further analysis of the security of the scheme discussed later.
...
%Method introduced in a 2 sensor environment before expansion to multiple sensor environments.
