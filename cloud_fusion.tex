...
\end{equation}
What remains is to compute the weights $\omega_i$ at the cloud such that the $n$-sensor FCI condition \eqref{eq:prelims:fci_matrix_equation} is met. 

Similar to the two-sensor case, we can use sequences of order-revealing encryptions to leak an approximation to this result. 

these weights by considering each constraint in 
as an independent partial solution before taking their intersection.

use the order-revealing method to solve each row of \eqref{eq:prelims:fci_matrix_equation} independently obtaining a linear subspace that is a partial solution of the total constraints. Taking the intersection of all partial solutions produces an estimate of the total solution to \eqref{eq:prelims:fci_matrix_equation}, namely the weights $\omega_i$

\begin{equation}\label{eq:cloud_fusion:secfci_nsen_partial_sol_subspace}
    \omega_i\tr(\mat{P}_{k,i}) = \omega_{i+1}\tr(\mat{P}_{k,i+1})\,,
\end{equation}
for $1\leq i<n$, $\omega_i+\omega_{i+1} = 1-\sum_{j\notin \{i,i+1\}}\omega_j$

The total solution subspace of \eqref{eq:cloud_fusion:secfci_nsen_partial_sol_subspace} can be defined by n-1 independent solutions. Solutions $\omega_j=1$ when $j\neq i,i+1$ are trivial (all others $0$). Remaining solution uses ORE
