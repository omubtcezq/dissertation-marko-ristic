...
def two_sen():
    fig = plt.figure()
    # 80% text width and default matplotlib aspect ratio
    fig.set_size_inches(w=0.8*5.78853, h=0.75*0.8*5.78853)
    ...
    ax.plot(gs, A, marker='.', c='tab:green', label=r'$\omega_1\tr(\mat{P}_1)$', zorder=3)
    ax.plot(gs, B, marker='.', c='tab:blue', label=r'$(1-\omega_1)\tr(\mat{P}_2)$', zorder=3)

    ax.scatter([l,r],[0, 0], marker='x', c='tab:grey', zorder=2, label=r'Solution Bounds')
    ax.plot([l, l],[0, (1-l)*trB], linestyle='--', c='tab:grey', zorder=2)
    ax.plot([r, r],[0, r*trA], linestyle='--', c='tab:grey', zorder=2)
    ax.scatter([0.5*(l+r)],[0], marker='x', c='tab:red', zorder=1, label=r'Approximate Solution')
...
def one_p_sol():
    ...
    ax.plot_trisurf(triangles, z, color='tab:grey', alpha=0.5, zorder=1)


    # Partial solution 1
    sol1 = 0.24
    sol1Line, = ax.plot([sol1, 0],[1-sol1, 0],[0, 1], linestyle='--', c='tab:green')
    ax.scatter([sol1, 0],[1-sol1, 0],[0, 1], c='tab:green', marker='x', depthshade=False)
    ...
    numpoints=1, loc=1, framealpha=0.6)
