...
A simulation of the presented privileged estimation scheme is presented here in addition to the theoretical backing. As in previous chapters, we simulated the two-dimension time-invariant constant velocity system model,
...
\begin{equation}\label{eq:priv_estimation:est_simulation_measurement_model_unbounded}
    ...
    \vec{x}_k + \vec{v}_k
\end{equation}
and the same values for $\mat{R}_k$ and $\mat{Z}$ as in the bounded case. Figure \ref{fig:priv_estimation:est_sim_bounded} shows the average error covariance traces and MSE of estimation from $1000$ simulation with models \eqref{eq:priv_estimation:est_simulation_system_model} and \eqref{eq:priv_estimation:est_simulation_measurement_model_unbounded} and shows similar results.
\begin{figure}[htbp]
   \centering
